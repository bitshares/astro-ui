...
const _internal = {
  ...
  decimal_precision_string(
    ...
    const number_string = v.to_string(number);
    const trimmed = number_string.trim();
    precision = v.to_number(precision);

    // remove leading zeros (not suffixing)
    const number_parts = trimmed.match(/^-?0*([0-9]*)\.?([0-9]*)$/);
    if (!number_parts) {
      throw new Error(`Invalid number: ${trimmed} ${error_info}`);
    }
    ...
    if (sign === "-" && !/[1-9]/.test(int_part + decimal_part)) {
      ...
      // (sign already "", but keep explicit for clarity)
    }
    if (int_part === "") {
      int_part = "0";
    }
...
const _my = {
  ...
  to_bigint64(
    ...
    return _internal.to_long64(number_or_string, precision, error_info);
  },
  ...
  to_string64(
    ...
    const number_long = v.to_long(number_or_string, error_info);
